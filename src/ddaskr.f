...
C$$$      SUM = 0.0D0
C$$$      DO 20 I = 1,NEQ
C$$$20      SUM = SUM + ((V(I)*RWT(I))/VMAX)**2
C$$$      DDWNRM = VMAX*SQRT(SUM/NEQ)
C$$$30    CONTINUE
C$$$      RETURN
C$$$C
C$$$C------END OF FUNCTION DDWNRM-------------------------------------------
C$$$      END

C slight re-write daniel kiracofe 7/29/09.  this routine gets
C called a lot.  rewrite makes it about 20% faster.
      ...
      DO 10 I = 1,NEQ
        tmp(I) = V(I)*RWT(I)
        IF (ABS(tmp(I)) .GT. VMAX) VMAX = ABS(tmp(I))
10    CONTINUE
