...
private  DFTF, fft1, mfft1, sfft

CONTAINS


!don't care if it is slow, don't call it too often
!returns a one sided spectrum
  subroutine my_fft(a, ierr)
    real*8, intent(inout) :: a(:)
    integer, intent(out) :: ierr
    real*8 :: b(size(a)), r(size(a))
    integer :: i, n

    b = 0

    n = size(a)
    call fft1(a,b, n, 1, ierr)

    if (ierr == 3) then
       !n has prime factors that the present fft routine doesn't like.  
       !just do the full DFT.  don't care if it is slow.
  ...
  subroutine my_dft( a, b, n)
    ...
    call DFTF(ac, c, N, -floor(dble(N)/2), ceiling(dble(N)/2) )

    a = real( c )
    b = aimag( c )
  end subroutine my_dft

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11
! code downloaded from http://www.phys.ufl.edu/~coldwell/Fourier/DFT%20code.htm
!with slight modifications to fortran 90 style by daniel

  SUBROUTINE DFTF(DAT,FSUM,N,M1,M2)
    integer, intent(in) :: n, m1, m2
    COMPLEX*16 DAT(*),FSUM(*),TEMP, ARG
    integer :: im, ib, ie,m, idat, i
    ...
    DO M=M1,M2
       ...
       IDAT=1
       DO I=IB,IE          
          ARG=(2*PI*I*M)/N !Note that the π makes the argument in parenthesis double precision so that the division by N is not integer math.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11
!downloaded from http://jblevins.org/mirror/amiller/fft.f90
